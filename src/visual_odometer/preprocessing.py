...
def apply_spatial_window(img, method: str, params: dict):
    ...
    if method == "blackman_harris":
        return apply_blackman_harris_window(img, params['a0'], params['a1'], params['a2'], params['a3']).astype(xp.float32)
    elif method == "raised_cosine":
        return apply_raised_cosine_window(img).astype(xp.float32)
...
def apply_downsampling(img, method: str, params: dict):
    xp = xp_backend()
    ...
    if method == "NN":
        return xp.array(img_pil.resize(newsize, Image.NEAREST))
    elif method == "bilinear":
        return xp.array(img_pil.resize(newsize, Image.BILINEAR))
    elif method == "bicubic":
        return xp.array(img_pil.resize(newsize, Image.BICUBIC))
    else:
        return img

def apply_frequency_window(spectrum: xp_backend().ndarray, method: str, params: dict):
...
def image_preprocessing(img, configs: dict):
    xp = xp_backend()
    ...
    img_spectrum = xp.fft.fftshift(xp.fft.fft2(img))
