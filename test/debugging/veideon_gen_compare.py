...
from virtualencoder.visualodometry.debugging_tools import debugging_estimate_shift, generate_plot_variables, \
    have_number_out_range, compare_two_variables_genimgs
...
for j in range(1):
    for i in tqdm(range(0, len(imgs) - 1, 1)):  # Itera sobre índices ímpares
        ...
        deltay, deltax, variables_1 = debugging_estimate_shift(img_0, img_1, finge_filter=False, finge_filter_threshold=0.03)
        additional_info = {
            "img_0_id": i,
            "img_1_id": i + 1,
        }
        variables_1.update(additional_info)

        deltay2, deltax2, variables_2 = debugging_estimate_shift(img_0, img_1, finge_filter=True, finge_filter_threshold=0.03, random_state=1)
        ...
        text_title = f"deltax1: {deltax:.2f}, deltay1: {deltay:.2f}\n deltax2: {deltax2:.2f}, deltay2:{deltay2:.2f}"
        plt = compare_two_variables_genimgs(img_0, img_1, variables_1, variables_2, text_title)
