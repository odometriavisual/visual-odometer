...
def apply_projection_phase_filter(CPS, R: int, dx_min: float, dx_max: float, dy_min: float,
                                  dy_max: float, keep_dim: bool = False):
    r"""
...
def proj_svd_method(fft_beg,
                    fft_end,
                    M: int, N: int,
                    R: int = 6,
                    dx_min: int = 0, dx_max: int = 64,
                    dy_min: int = 0, dy_max: int = 48,
                    phase_windowing=None,
                    unwrap_method="itoh1982",
                    svd_method = "svds",
                    **svd_kwargs) -> tuple[float, float]:
    """
    Estimate displacement using projection-based phase correlation.
    ...
    **svd_kwargs : dict
    ...
    def apply_projection_phase_filter(CPS, R, dx_min, dx_max, dy_min, dy_max, keep_dim=False):
        # Sua implementação original
        Cn_t = np.fft.ifft2(CPS)
        M, N = Cn_t.shape
        delta_x = (dx_max - dx_min) + R
        delta_y = (dy_max - dy_min) + R
        lb_y = np.clip(M // 2 - delta_y // 2, a_min=0, a_max=M)
        ub_y = np.clip(M // 2 + delta_y // 2, a_min=0, a_max=M)
        lb_x = np.clip(N // 2 - delta_x // 2, a_min=0, a_max=N)
        ub_x = np.clip(N // 2 + delta_x // 2, a_min=0, a_max=N)
        ...
        if keep_dim:
            ...
            return np.fft.fftshift(np.fft.fftshift(Cn_t)[lb_y: ub_y, lb_x: ub_x])
    ...
    qu, s, qv = compute_dominant_singular_vectors(Q_filtered, method=svd_method, **svd_kwargs)
